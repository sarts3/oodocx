...
from helper_functions import remove_readonly, add_relationship, get_image_size
from write_files import write_rels, setup_comments, write_content_types
...
class Docx():
    def __init__(self, file=''):
        ...
        self.xmlfiles = {}
        try:
            shutil.rmtree(self.write_dir, onerror=remove_readonly)
        except:
            pass
        ...
        if file:
            ...
            self.rels = write_rels()
            self.xmlfiles[self.rels] = os.path.join('_rels', '.rels')

            self.contenttypes = write_content_types()
...
def picture(document, picpath, picdescription='', pixelwidth=None, pixelheight=None, nochangeaspect=True, nochangearrowheads=True):
    ...
    if not pixelwidth or not pixelheight:
        pixelwidth, pixelheight = get_image_size(picpath)
...
def add_comment(document, text, start, end=None, username='', initials=''):
    if end is None:
        ...
        eggparent.index(egparent)):
            raise ValueError('end element cannot precede start element')
    id_number = setup_comments(document)
